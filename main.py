...
'''
        ...
        path = constants.path + "pic\\"
        color = '#5e0d84'
        if new_schedule is True:
            with open(path + 'rasp.xlsx', 'wb') as f:
                f.write(bot.download_file(bot.get_file(message.document.file_id).file_path))
        cursor.execute("SELECT data FROM switch WHERE field = 'month_this' or field = 'month_past' ")  # past this
        months_filled = cursor.fetchall()
        if months_filled[1][0] is not '':
            doc = bot.download_file(bot.get_file(months_filled[1][0]).file_path)
        else:
            bot.reply_to(message, '*УСТАНОВИТЕ НОВЫЙ ФОН*', parse_mode='Markdown')
            doc = bot.download_file(bot.get_file(months_filled[0][0]).file_path)
        with open(path + 'month.jpg', 'wb') as f:
            f.write(doc)
        base = Image.open(path + 'month.jpg').convert('RGBA')
        fnt_lessons = ImageFont.truetype(path + "micross.ttf", 40)
        fnt_group = ImageFont.truetype(path + "micross.ttf", 96)
        fnt_day = ImageFont.truetype(path + "micross.ttf", 50)
        wb = openpyxl.load_workbook(filename=path + 'rasp.xlsx')
        sheet = wb['доска']
        for day in a:
            bot.send_message(message.chat.id, "Устанавливается фон на " + translate[day][0], disable_notification=True)
            for column in range(2, 8, 2):
                for name in range(translate[day][1], translate[day][1] + 63, 9):
                    y = 200  # 170
                    txt = Image.new('RGBA', base.size, (255, 255, 255, 0))  # text base
                    d = ImageDraw.Draw(txt)
                    klass = sheet[str(columns[column]) + str(name)].value
                    if klass is not None:  # group cell
                        schedule = ""
                        d.text((10, 745), translate[day][0], font=fnt_day, fill=(255, 255, 255))  # write day
                        klass_regexp = re.compile(r'[А-Яа-я]{1,2}\s*\d+')  # 1 или 2 буквы; любые пробелы; 1+ цифры
                        klass = klass_regexp.search(klass).group(0).replace(" ", '')  # Убрать всякие пробелы
                        d.text((535, 30), klass, font=fnt_group, fill=(255, 255, 255))  # write group  600 80
                        vals = sheet[str(columns[column]) + str(name + 1) + ':' + str(columns[column + 1]) + str(name + 8)]  # schedule
                        num = 1
                        for val in vals:
                            subj = '' if val[0].value is None else str(num) + '. ' + str(val[0].value)  # subject
                            croom = '' if val[1].value is None else str(int(val[1].value)) if isinstance(val[1].value, float) else str(val[1].value)  # classroom
                            if subj is not None:
                                d.text((300, y), subj, font=fnt_lessons, fill=color)
                                d.text((900, y), croom, font=fnt_lessons, fill=color)
                                schedule += subj + "\n"
                                y += 70
                                num += 1
                        out = Image.alpha_composite(base, txt)
                        out.save(path + 'temp.png', "PNG")
                        time.sleep(3.5)
                        with open(path + "temp.png", 'rb') as f:
                            photo = bot.send_photo(-1001079895757, f, disable_notification=True).photo[-1].file_id
                            cursor.execute("UPDATE schedule SET {day} = '{photo_id}' WHERE klass = '{group}'".format(day=day,
                                                                                                                     photo_id=photo,
                                                                                                                     group=klass))
                            # klass строго должен подходить под regexp r'[А-Я]{1,2}\d+'
        conn.commit()
...
@bot.callback_query_handler(func=lambda call: True)
# =========================================================================================================== VOTE_YES
        if call.data == "vote_yes":  # Голос!
            functions.vote(call, 'ЗА')
# =========================================================================================================== VOTE_NO
        if call.data == "vote_no":
            functions.vote(call, 'ПРОТИВ')
# =========================================================================================================== VOTE_CHANGE
        if call.data == "vote_change":  # Смена голоса
            conn = functions.start_sql()
            cursor = conn.cursor(buffered=True)
            select_query = 'SELECT vote FROM known_users WHERE id = {0}'.format(str(call.from_user.id))
            cursor.execute(select_query)
            for user_vote in cursor.fetchone():  # Получение нового голоса!
                if user_vote == 'ЗА':
                    new_vote = 'ПРОТИВ'
                else:
                    new_vote = 'ЗА'
            query = "UPDATE known_users SET vote = '{0}' WHERE id = {1}".format(new_vote, str(call.from_user.id))  # Вставка голоса в бд
            cursor.execute(query)
            conn.commit()
            cursor.close()
            conn.close()
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='Ваш голос: '+new_vote)
# =========================================================================================================== FIRST VOTE
        if call.data == "vote_start":  # Показ стартового сообщения
            conn = functions.start_sql()
            cursor = conn.cursor(buffered=True)
            select_query = 'SELECT vote FROM known_users WHERE id = {0}'.format(str(call.from_user.id))
            cursor.execute(select_query)
            for user_vote in cursor.fetchone():
                text = constants.vote.format(call.from_user.first_name)+"\n\nВаш голос: "+user_vote
            keyboard_change = telebot.types.InlineKeyboardMarkup()
            keyboard_change.add(telebot.types.InlineKeyboardButton(text="Я хочу изменить свой голос", callback_data='vote_change'))
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=text, reply_markup = keyboard_change)
'''
